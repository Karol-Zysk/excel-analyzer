                    ...
                    <div className="space-y-5">
                      ...
                      <div className="space-y-[31px]">
                        {mappedImport.invoices.map((invoice) => {
                          ...
                          return (
                            <InvoiceDraftCard
                              key={`${invoice.invoiceNumber}-${invoiceKey}`}
                              invoice={invoice}
                              invoiceKey={invoiceKey}
                              invoiceDraft={invoiceDraft}
                              myCompanyRole={myCompanyRole}
                              buyerIdentifierType={defaults.buyerIdentifierType}
                              copiedInvoiceItems={copiedInvoiceItems}
                              singleInvoiceError={singleInvoiceErrorByKey[invoiceKey]}
                              isSingleInvoicePending={
                                singleInvoiceMutation.isPending &&
                                singleInvoiceMutation.variables?.invoiceKey ===
                                  invoiceKey
                              }
                              submitSingleInvoice={submitSingleInvoice}
                              updateInvoiceOverrideField={updateInvoiceOverrideField}
                              updateInvoiceOverrideItemField={
                                updateInvoiceOverrideItemField
                              }
                              addInvoiceOverrideItem={addInvoiceOverrideItem}
                              removeInvoiceOverrideItem={removeInvoiceOverrideItem}
                              copyInvoiceItems={copyInvoiceItems}
                              applyCopiedItemsToInvoice={applyCopiedItemsToInvoice}
                            />
                          );
                        })}
                      </div>
